...
Sprint 3.8 connects the camera adapter, lifecycle coordinator, and barcode decoders to the ScannerClient React view under `src/app/(game)/scan/scanner-client.tsx`. The view owns `HTMLVideoElement` preview, real-time decode loop, scan guard with cooldown and duplicate filtering, camera device enumeration, camera switching, stream lifecycle cleanup on unmount, and status-based UI state transitions. It must not implement product lookup, feeding, pet updates, rewards, achievements, missions, or inventory processing.

Sprint 3.9 adds the Mobile Scanner Optimization module under `src/lib/scanner/mobile-optimization.ts`. This module provides device-tier detection (`MobileDeviceHint`), per-device `getUserMedia` constraints (including iPhone 11 blurry ultra-wide mitigation), continuous autofocus request via `applyConstraints`, battery-aware decode interval scaling, Page Visibility API background/foreground recovery, `screen.orientation` change handling, Safari and Android Chrome compatibility helpers, `disposeMediaStream` and `releaseVideoElement` memory cleanup, and `configureVideoForMobileScanning` renderer attributes. The `BrowserCameraAdapter.createStream` now delegates constraint building to `buildMobileCameraConstraints`. The ScannerClient integrates visibility recovery, orientation handling, battery-aware decode intervals, and the full `executeScanFeedFlow` pipeline for end-to-end scan→feed→pet→XP processing. This module must not own camera streams, barcode decoding, product lookup, feeding, pet state, rewards, or UI rendering.

Sprint 4.1 adds the Inventory Domain Foundation under `src/lib/inventory/engine.ts` and `src/types/inventory.ts`. The engine is a pure, deterministic function library with zero dependencies on React, browser APIs, Prisma, repositories, services, or stores. It provides `createInventory`, `validateItem`, `addItem` (stackable with automatic metadata merge), `removeItem` (with slot pruning at zero quantity), `updateItemQuantity`, `sortInventory` (deterministic by type/quantity/itemKey/id with ascending/descending order), `serializeInventory`, and `deserializeInventory`. Domain models include `InventoryModel` (with capacity), `InventoryItem` (with type, itemKey, quantity, metadata), `InventoryItemType`, `InventoryEngineResult`, and `InventoryEngineError`. The `InventoryRepository` interface exposes `getInventoryByUserId` and `saveInventory`, implemented by `PrismaInventoryRepository` using transaction-based upsert with delete sync. `InventoryService` orchestrates `getInventory`, `addItem`, and `removeItem` through the pure engine and repository. `useInventoryStore` delegates mutations to engine functions without embedding business logic. The engine must not import React, browser APIs, Prisma, repositories, services, stores, or UI components.
